                               ...
                               int section_bar)
{
    ...
    uint32_t seed_local =
        g_seed ^ (uint32_t)part ^ (uint32_t)section_bar;
    ...
    for (int i = 0; i < steps_count; i++)
    {
        ...
        ft_step_t s = {
            ...
            .step     = (uint8_t)i,
            .pitch    = (uint8_t)(r % 128),
            .velocity = (uint8_t)((r >> 1) & 0x7F),
            .gate     = (uint8_t)((r & 0x7) ? 127 : 0)
        };
...
void randomizer_init_default(void)
{
    ...
    for (int i = 0; i < RANDOMIZER_MAX_MOTION_LANES; i++)
    {
        ...
        memset(g_motions[i].values, 0, sizeof(g_motions[i].values));
    }
